...
- [[#fix][Fix]]
  ...
  - [[#org-journal-not-in-the-right-language][Org journal not in the right language]]
  ...
  (pretty-hydra-define my/hydra-go-to-folder
    ...
    ("Subtitle"
     (("t" (counsel-find-file "/tmp/") "Tmp")
      ...
      ("pj" (counsel-find-file "~/Documents/Projects") "Projects")
...
#+begin_src emacs-lisp :tangle yes :results silent
  (pretty-hydra-define my/hydra-Go-To-Notes
    ...
    (
     "DevOps"
     (("li" (find-file "~/Notes/Linux/linux.org") "Linux")
      ("do" (find-file "~/Notes/Docker/docker.org") "Docker"))
     "Programming"
     (("py" (find-file "~/Notes/Python/python.org") "Python")
      ("an" (find-file "~/Notes/Android/android.org") "Android")
      ("sh" (find-file "~/Notes/Bash/bash.org") "Bash")
      ("cm" (find-file "~/Notes/Cmake/cmake.org") "Cmake")
      ("mf" (find-file "~/Notes/Makefile/makefile.org") "Makefile")
      ("js" (find-file "~/Notes/Javascript/javascript.org") "Javascript")
      ("ml" (find-file "~/Notes/Matlab/matlab.org") "Matlab"))
     "Others"
     (("dl" (find-file "~/Notes/MachineLearning/machineLearning.org") "Deep Learning"))
     ))
...
***** Go to Agenda
#+begin_src emacs-lisp :tangle yes :results silent
  (pretty-hydra-define my/hydra-Go-To-Agenda
    (:hint nil :foreign-keys warn :title "Go-To-Notes" :quit-key "q" :color blue)
    ("GTD"
     (("in" (find-file "~/Org/GTD/inbox.org") "Inbox")
      ("mb" (find-file "~/Org/GTD/maybe.org") "Maybe")
      ("pj" (find-file "~/Org/GTD/projects.org") "Projects")
      ("ta" (find-file "~/Org/GTD/tasks.org") "Tasks")
      ("ti" (find-file "~/Org/GTD/tickler.org") "Tickler")
      ("to" (find-file "~/Org/GTD/today.org") "Today")
      ("ar" (find-file "~/Org/GTD/archive.org") "Archive"))
     "Sorted"
     (("im" (find-file "~/Org/Other/improve.org") "Improve")
      ("lr" (find-file "~/Org/Other/learning.org") "Learning")
      ("li" (find-file "~/Org/Other/links.org") "Links")
      ("or" (find-file "~/Org/Other/orders.org") "Orders")
      ("tt" (find-file "~/Org/Other/commands.org") "Commands")
      ("mo" (find-file "~/Org/Other/movies.org") "Movies"))
     "Organisation"
     (("cl" (find-file "~/Org/GTD/recurrent_checklist.org") "Check list"))
     "Journal"
     (("jo" (find-file "~/Org/Journal/summary.org") "Journal"))
     ))

  (global-set-key (kbd "C-z C-a") 'my/hydra-Go-To-Agenda/body)
#+end_src
***** Go to Projects
#+begin_src emacs-lisp :tangle yes :results silent
  (pretty-hydra-define my/hydra-Go-To-Project
    (:hint nil :foreign-keys warn :title "Go-To-Notes" :quit-key "q" :color blue)
    (
     "Installing script"
     (("mj" (find-file "~/Documents/Projects/Install-scripts/Manjaro/Install_manjaro.org") "Manjaro.org"))
     ))

  (global-set-key (kbd "C-z C-p") 'my/hydra-Go-To-Project/body)
#+end_src
***** Set Workspace mode
#+begin_src emacs-lisp :tangle yes :results silent
  ...
  (defun cn/startup-state ()
    ...
    (set-one-file-to-window "~/Dotfiles/Xmonad/.xmonad/xmonad.hs")
  ...
  (use-package org-faces
    ...
    (org-todo-keyword-faces
     '(
       ...
       ("DELEGATED" . (:foreground "light sky blue" :weight bold))
...
#+begin_src emacs-lisp :tangle yes :results silent
  (use-package org-capture
    ...
    (defvar my/org-full-task-template "* TODO [#%^{priority|1|2|3|4|5}] %^{Tasks}
    ...
    (defvar my/org-basic-tickler-template "* TODO [#%^{priority|1|2|3|4|5}] %^{Tasks}
                 SCHEDULED:%(org-insert-time-stamp (org-read-date nil t) nil nil nil nil \" .+1d\")
                 Captured %<%Y-%m-%d %H:%M>" "Template for basic task.")
    (defvar my/org-basic-task-today-template "* TODO %^{Tasks}
              :SCHEDULED: <%<%Y-%m-%d %a>>
              Captured %<%Y-%m-%d %H:%M>" "Template for basic task.")
    (defvar my/org-basic-task-template "* TODO %^{Tasks}
                 Captured %<%Y-%m-%d %H:%M>" "Template for basic task.")

    (defvar my/org-basic-project-template "**  %^{Project} [%]
    Captured %<%Y-%m-%d %H:%M>" "Template for maybe project.")
    ...
    (org-capture-templates
     `(
       ...
       ("T" "Task full" entry (file+headline "~/Org/GTD/tickler.org" "Tasks"),
        my/org-full-task-template
        :empty-lines 1)

       ("a" "Task today" entry (file+headline "~/Org/GTD/tickler.org" "Tasks"),
        my/org-basic-task-today-template
        :empty-lines 1)

       ("t" "Task basic" entry (file+headline "~/Org/GTD/tasks.org" "Tasks"),
       ...
       ("i" "Random" entry (file+headline "~/Org/GTD/inbox.org" "inbox"),
        my/org-basic-task-template
        :empty-lines 1)
       ...
       ("j" "Journal" entry (file+datetree "~/Org/Journal/summary.org"),
        my/journal-template
        :empty-lines 1)
...
#+begin_src emacs-lisp :tangle yes :results silent
  ...
  (setq org-agenda-files (list "~/Org/GTD/tickler.org"
                               "~/Org/GTD/recurrent_checklist.org"
                               "~/Org/GTD/projects.org"
                               "~/Org/GTD/today.org"
                                 ))
  (setq org-todo-keywords
        '((sequence "TODO(t)" "NEXT(n)" "WAITING(w@/!)" "|" "DONE(d!)" "CANCELED(c@/!)" "DELEGATED(m@/!)")
...
package-manage)
#+begin_src emacs-lisp :tangle yes :results silent
  (setenv "PATH" "/usr/local/texlive/2021/bin/x86_64-linux:$PATH" t)
  ...
  (setq TeX-show-compilation t)
  (setq org-latex-logfiles-extensions (quote ("aux" "bcf" "blg" "fdb_latexmk" "fls" "figlist" "idx" "log" "nav" "out" "ptc" "run.xml" "snm" "toc" "vrb" "xdv" "synctex.gz" "bbl")))
#+end_src
**** COMMENT Org-ref
#+begin_src emacs-lisp :tangle yes :results silent
  (use-package org-ref)
...
#+begin_src emacs-lisp :tangle yes :results silent
  (use-package eyebrowse
    ...
    :bind(
          ...
          ("C-z &" . eyebrowse-switch-to-window-config-0)
          ("C-z é" . eyebrowse-switch-to-window-config-1)
          ("C-z \"" . eyebrowse-switch-to-window-config-2)
          ("C-z '" . eyebrowse-switch-to-window-config-3)
          ("C-z (" . eyebrowse-switch-to-window-config-4)
          ("C-z §" . eyebrowse-switch-to-window-config-5)
          ("C-z è" . eyebrowse-switch-to-window-config-6)
          ("C-z !" . eyebrowse-switch-to-window-config-7)
          ("C-z ç" . eyebrowse-switch-to-window-config-8)
          ("C-z à" . eyebrowse-switch-to-window-config-9)
          )
    )
...
#+begin_src emacs-lisp :tangle yes :results silent
  ...
  ;; :ensure t
  ;; :config(disable-mouse-global-mode 1))
#+end_src
...
#+begin_src emacs-lisp :tangle yes :results silent
  ...
  ;;     (cfw:ical-create-source "gcal" "https://calendar.google.com/calendar/b/2?cid=Y2xlbWVudC5uaWNvbGFzLjE5OTZAZ21haWwuY29t" "Red") ; devorah calender
  ;;     )))
